...
func TestBuildTemplate(t *testing.T) {
	tests := []struct {
		...
		assert func(t *testing.T, rc *run.Context, tmp string, got *build.Template, err error)
	}{
		{
		...
		},
		{
			...
			assert: func(t *testing.T, rc *run.Context, tmp string, got *build.Template, err error) {
				...
				assert.IsType(t, template.NewCopyImageFileAction(image, rc), got.Prebuild)
				got.Prebuild = nil
